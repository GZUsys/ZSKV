...
namespace ROCKSDB_NAMESPACE {
  ...
  CompactionIterator::CompactionIterator(
    ...
    bool enforce_single_del_contracts,
    const std::atomic<bool>& manual_compaction_canceled,
    bool must_count_input_entries, SegmentManager* segment_manager,
    ...
    : CompactionIterator(
      ...
      report_detailed_time, expect_valid_internal_key, range_del_agg,
      blob_file_builder, allow_data_in_errors, enforce_single_del_contracts,
      manual_compaction_canceled, segment_manager,
      compaction ? std::make_unique<RealCompaction>(compaction) : nullptr,
      must_count_input_entries, compaction_filter, shutting_down, info_log,
      full_history_ts_low, preserve_seqno_min) {
  }
  ...
  void CompactionIterator::NextFromInput() {
    ...
    while (!Valid() && input_.Valid() && !IsPausingManualCompaction() &&
      ...
      // 判断当前user_key是否和上一个相同（忽略ts），以及ts比较
      ...
      // 遇到了一个新的用户 key（可以是 key 本身不同，或 ts 不同）
  ...
  void CompactionIterator::delete_value_from_zns(Slice value) {
    if (value.data()[1] == is_value_separated) {
      value.remove_prefix(2);
      uint64_t length = 0;
      uint64_t segment_id = 0;
      uint64_t start_page = 0;
      GetVarint64(&value, &length);
      GetVarint64(&value, &segment_id);
      GetVarint64(&value, &start_page);    
      LBA lba(segment_id, start_page);
      segment_manager_->delete_data(lba, length);
    }
