...
Writer::Writer(std::unique_ptr<WritableFileWriter>&& dest, uint64_t log_number,
               bool recycle_log_files, SegmentManager* segment_manager, bool manual_flush,
...
IOStatus Writer::write_log(const WriteOptions& write_options,WriteBatch* batch, const SequenceNumber& seqno){
  IOStatus s = MaybeHandleSeenFileWriterError();
  if (!s.ok()) {
    return s;
  }
  PPA ppa;
  LBA lba;
  for (const auto& pair : batch->get_segment_data()) {
    ...
    if (segment_id == config::max_open_zones) {
      ...
      if (s.ok()) {
        ...
        char* heard = new char[12];
        uint64_t length = data.size();
        EncodeFixed64(heard + 4, data.size());
        uint32_t crc = crc32c::Value(data.data(), data.size());
        crc = crc32c::Mask(crc);
        EncodeFixed32(heard, crc);
        s = dest_->Append(opts, Slice(heard), 0);
        s = dest_->Append(opts, Slice(data), 0);
      }
      if (s.ok()) {
        s = dest_->Flush(opts);
      }
    } else {
      segment_manager_->write_to_segment(segment_id, data.c_str(), data.size(),
                                       ppa, lba);
      batch->Append_LBA(segment_id, lba);
      batch->Append_PPA(segment_id, ppa);
    }
  }
