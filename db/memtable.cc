...
bool MemTable::Get(const LookupKey& key, std::string* value,
  ...
  if (type == not_value_separated) {
    ...
    if (!GetVarint64(&val, &length)) {
      *s = Status::Corruption("bad internal key5");
      return false;
    }
    if (!GetVarint64(&val, &segment_id)) {
      *s = Status::Corruption("bad internal key6");
      return false;
    }
    if (!GetVarint64(&val, &start_page)) {
      *s = Status::Corruption("bad internal key7");
      return false;
    }
    value->assign(val.data(), val.size());
  } else {
    *s = Status::Corruption("bad internal key8");
    return false;
  }
  }
