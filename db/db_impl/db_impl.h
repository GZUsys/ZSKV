...
#include "rocksdb/AsyncPrefetchThreadPool.h"
...
namespace ROCKSDB_NAMESPACE {
  ...
  public:
    ...
    bool ResolveSeparatedValue(Slice& value_, Slice& real_value);
    ...
    using DB::Rangeget;
    Status Rangeget(const Slice start_key,
      int key_num, const ReadOptions& options,
      std::vector<Slice>& keys, std::vector<Slice>& vals) override;
  ...
  private:
    ...
    IOStatus WriteToWAL(WriteBatch& merged_batch,
      const WriteOptions& write_options,
    ...
    ZNSManager* zns_manager_;
    std::unique_ptr<AsyncPrefetchThreadPool> async_thread_pool_;
  };
