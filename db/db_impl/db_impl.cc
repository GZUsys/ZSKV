...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
  ...
  l2p_map_ = new L2PMap();
  zns_manager_ = new ZNSManager(l2p_map_);
  segment_manager_ = new SegmentManager(zns_manager_);
  async_thread_pool_ = std::make_unique<AsyncPrefetchThreadPool>(8);
...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  if (!done) {
    ...
    if (s.IsNotFound()) {
      ...
      Slice val = *get_impl_options.value;
        if (val.size() > 0) {
          is_kv_separated type =
              static_cast<is_kv_separated>(static_cast<uint8_t>(val.data()[1]));
          val.remove_prefix(2);
          if (type == not_value_separated) {
            get_impl_options.value->Reset();
            get_impl_options.value->PinSelf(val);
          } else if (type == is_value_separated) {
            uint64_t length = 0;
            uint64_t segment_id = 0;
            uint64_t start_page = 0;
            if (!GetVarint64(&val, &length) ||
                !GetVarint64(&val, &segment_id) ||
                !GetVarint64(&val, &start_page)) {
              s = Status::Corruption("bad internal key9");
              return s;
            }
            ...
            Slice key, real_value;
            if (GetLengthPrefixedSlice(&buffer_slice, &key) &&
                GetLengthPrefixedSlice(&buffer_slice, &real_value)) {
                  get_impl_options.value->Reset();
                  get_impl_options.value->PinSelf(Slice(real_value.data(), real_value.size()));
            } else {
              s = Status::Corruption("bad WriteBatch Put");
            }
          } else {
            s = Status::Corruption("bad internal key10");
          }
        }

    }
      // if (get_impl_options.value_found && *get_impl_options.value_found) {
      //   Slice val = *get_impl_options.value;
      //   if (val.size() > 0) {
      //     is_kv_separated type =
      //         static_cast<is_kv_separated>(static_cast<uint8_t>(val.data()[0]));
      //     val.remove_prefix(1);
      //     if (type == not_value_separated) {
      //       get_impl_options.value->Reset();
      //       get_impl_options.value->PinSelf(val);
      ...
      //       char* buffer = new char[length + 17];
      //       zns_manager_->read_from_ppa(ppa, buffer, length + 17);
      //       Slice buffer_slice(buffer + 17, length);
      //       delete[] buffer;
...
Iterator* DBImpl::NewIterator(const ReadOptions& _read_options,
  ...
  if (read_options.tailing) {
    ...
    result = DBIter::NewIter(env_, read_options, cfd->ioptions(),
                             ...
                             /*read_callback=*/nullptr, /*active_mem=*/nullptr,l2p_map_,zns_manager_,
...
ArenaWrappedDBIter* DBImpl::NewIteratorImpl(
  ...
  return NewArenaWrappedDbIterator(
      ...
      expose_blob_index, allow_refresh, /*allow_mark_memtable_for_flush=*/true,l2p_map_,
      zns_manager_);
}
