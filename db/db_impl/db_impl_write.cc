...
namespace ROCKSDB_NAMESPACE {
  ...
  Status DBImpl::Rangeget(const Slice start_key, int key_num, const ReadOptions& options, std::vector<Slice>& keys,
    ...
    std::vector<KeyAndFuture> async_kv;
    keys.clear();
    vals.clear();
    std::unique_ptr<Iterator> iter(NewIterator(options));
    if (!iter) {
      return Status::InvalidArgument("Failed to create iterator");
    }

    int count = 0;
    iter->Seek(start_key);
    for (; iter->Valid() && count < key_num; iter->Next(), ++count) {
      const Slice& k = iter->key();
      Slice v = iter->value();

      std::string value_copy = v.ToString();
      async_kv.push_back({
        k.ToString(),
        async_thread_pool_->enqueue([this, value_copy]() {
          Slice v_copy(value_copy);
          Slice real_val;
          if (!ResolveSeparatedValueAsync(v_copy, real_val,zns_manager_, l2p_map_)) {
            return std::make_pair(false, std::string());
          }
          return std::make_pair(true, real_val.ToString());
        })
        });
    }
    for (auto& kv : async_kv) {
      auto res = kv.future.get();
      if (!res.first) return Status::Corruption("ResolveSeparatedValueAsync failed");
  ...
  bool DBImpl::ResolveSeparatedValue(Slice& value_, Slice& real_value) {
      ...
      if (status.ok()) {
        ...
        if (!parallel) {
          ...
          w.status = WriteBatchInternal::InsertInto(
            ...
            &flush_scheduler_, &trim_history_scheduler_, *l2p_map_,
  ...
  IOStatus DBImpl::WriteToWAL(WriteBatch& merged_batch,
    const WriteOptions& write_options,
    ...
    *log_size = log_entry.size();
    ...
    io_s = log_writer->write_log(write_options, &merged_batch, sequence);
  ...
  IOStatus DBImpl::WriteGroupToWAL(const WriteThread::WriteGroup& write_group,
    ...
    std::vector<int> segment_wps = segment_manager_->get_all_segment_wp();
    for (auto writer : write_group) {
      writer->batch->Split_rep_to_map(config::min_value_size, s, segment_wps);
    }
    ...
    if (write_with_wal > 1 && io_s.ok()) {
      for (const auto& [id, lba] : merged_batch->get_LBA_info()) {
        write_group.leader->batch->Append_LBA(id, lba);
      }
      for (const auto& [id, ppa] : merged_batch->get_PPA_info()) {
        write_group.leader->batch->Append_PPA(id, ppa);
      }
    }
