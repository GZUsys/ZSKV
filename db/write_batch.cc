...
namespace ROCKSDB_NAMESPACE {
  ...
  Status WriteBatch::Iterate(Handler* handler, L2PMap& l2p_map,
    std::map<int, LBA>& lba_map, std::map<int, PPA>& ppa_map) const {
    if (rep_.size() < WriteBatchInternal::kHeader) {
      return Status::Corruption("malformed WriteBatch (too small)");
    }

    return WriteBatchInternal::Iterate(this, handler, l2p_map, lba_map, ppa_map);
  }
  ...
  Status WriteBatchInternal::Iterate(const WriteBatch* wb, WriteBatch::Handler* handler,
    L2PMap& l2p_map, std::map<int, LBA>& lba_map, std::map<int, PPA>& ppa_map) {
    for (const auto& pair : wb->get_segment_data()) {
      Status s;
      int segment_id = pair.first;
      const std::string& data = pair.second;
      PPA ppa; LBA lba;
      if (segment_id < config::max_open_zones) {
        ppa = ppa_map.find(segment_id)->second;
        lba = lba_map.find(segment_id)->second;
      }
      Slice input(data);
      if (input.size() < 17) {
        return Status::Corruption("malformed WriteBatch (too small)");
      ...
      while (!input.empty()) {
        if (segment_id == config::max_open_zones) {
          ...
          uint64_t length = DecodeFixed64(header);
          SequenceNumber seq = DecodeFixed64(header + 8);
          input.remove_prefix(17);
          ...
          if (type == kTypeValue) {
            if (GetLengthPrefixedSlice(&input, &key) &&
              GetLengthPrefixedSlice(&input, &value)) {
              std::string value_with_type;
              value_with_type.push_back(1);                      
              value_with_type.push_back(not_value_separated);   
              value_with_type += value.ToString();
              handler->PutCF(seq, column_family, key, value_with_type);
            }
            else {
              return Status::Corruption("bad WriteBatch Put");
            }
            ...
            if (GetLengthPrefixedSlice(&input, &key)) {
              s = handler->DeleteCF(seq, column_family, key);
            }
            else {
              return Status::Corruption("bad WriteBatch Delete");
            }
          ...
          const char* header = input.data();
          const unsigned int type = header[32];
          uint64_t length = DecodeFixed64(header + 4);
          SequenceNumber seq = DecodeFixed64(header + 12);
          input.remove_prefix(33);
          if (type == kTypeValue) {
            if (input.size() < length) {
              return Status::Corruption("malformed WriteBatch (too small)");
            }
            ...
            if (GetLengthPrefixedSlice(&input, &key) &&
              GetLengthPrefixedSlice(&input, &value)) {
              l2p_map.insert(lba, ppa);
              std::string value_with_type;
              value_with_type.push_back(1);                      
              value_with_type.push_back(is_value_separated);   
              PutVarint64(&value_with_type, length);
              PutVarint64(&value_with_type, lba.segment_id);
              PutVarint64(&value_with_type, lba.start_page);
              value_with_type += value.ToString();
              handler->PutCF(seq, column_family, key, value_with_type);
              size_t raw = length + 33;
              size_t aligned = (raw + config::page_size - 1) & ~((size_t)(config::page_size - 1));
              size_t padding = aligned - raw;
              lba.set(lba.segment_id,
                lba.start_page + aligned / config::page_size);
              ppa.set(ppa.zone_id, ppa.start_page + aligned / config::page_size);
              input.remove_prefix(padding);
            }
            else {
              return Status::Corruption("bad WriteBatch Put");
            }
      ...
      lba_map[segment_id] = lba;
      ppa_map[segment_id] = ppa;
  ...
  namespace {
    ...
    public:
      ...
      template <typename RebuildTxnOp>
      Status PutCFImpl(SequenceNumber seq, uint32_t column_family_id, const Slice& key,
        const Slice& value, ValueType value_type,
        RebuildTxnOp rebuild_txn_op,
        const ProtectionInfoKVOS64* kv_prot_info) {
        // optimize for non-recovery mode
        ...
        if (!moptions->inplace_update_support) {
          ret_status =
            mem->Add(seq, value_type, key, value, kv_prot_info,
              concurrent_memtable_writes_, get_post_process_info(mem),
              hint_per_batch_ ? &GetHintMap()[mem] : nullptr);
        }
        else if (moptions->inplace_callback == nullptr ||
          value_type != kTypeValue) {
          assert(!concurrent_memtable_writes_);
          ret_status = mem->Update(seq, value_type, key, value, kv_prot_info);
        }
        else {
          assert(!concurrent_memtable_writes_);
          assert(value_type == kTypeValue);
          ret_status = mem->UpdateCallback(seq, key, value, kv_prot_info);
          if (ret_status.IsNotFound()) {
            // key not found in memtable. Do sst get, update, add
            SnapshotImpl read_from_snapshot;
            read_from_snapshot.number_ = sequence_;
              ...
              if (update_status == UpdateStatus::UPDATED_INPLACE) {
                ...
                if (kv_prot_info != nullptr) {
                  ...
                  updated_kv_prot_info.UpdateV(value,
                    Slice(prev_buffer, prev_size));
                  // prev_value is updated in-place with final value.
                  ret_status = mem->Add(seq, value_type, key,
                    Slice(prev_buffer, prev_size),
                    &updated_kv_prot_info);
                }
                else {
                ...
                if (kv_prot_info != nullptr) {
                  ...
                  // merged_value contains the final value.
                  ret_status =
                    mem->Add(seq, value_type, key, Slice(merged_value),
                      nullptr /* kv_prot_info */);
                }
                if (ret_status.ok()) {
                  RecordTick(moptions->statistics, NUMBER_KEYS_WRITTEN);
      ...
      Status PutCF(SequenceNumber seq, uint32_t column_family_id, const Slice& key,
        const Slice& value) override {
        const auto* kv_prot_info = NextProtectionInfo();
        Status ret_status;
        ...
        if (kv_prot_info != nullptr) {
          ...
          auto mem_kv_prot_info =
            kv_prot_info->StripC(column_family_id).ProtectS(sequence_);
          ret_status = PutCFImpl(seq, column_family_id, key, value, kTypeValue,
            rebuild_txn_op, &mem_kv_prot_info);
        }
        else {
          ret_status = PutCFImpl(seq, column_family_id, key, value, kTypeValue,
      ...
      Status DeleteCF(SequenceNumber seq, uint32_t column_family_id, const Slice& key) override {
        const auto* kv_prot_info = NextProtectionInfo();
        // optimize for non-recovery mode
        if (UNLIKELY(write_after_commit_ && rebuilding_trx_ != nullptr)) {
          // TODO(ajkr): propagate `ProtectionInfoKVOS64`.
        ...
        if (kv_prot_info != nullptr) {
          ...
          ret_status = DeleteImpl(seq, column_family_id, key, Slice(), delete_type,
            &mem_kv_prot_info);
        }
        else {
          ret_status = DeleteImpl(seq, column_family_id, key, Slice(), delete_type,
      ...
      Status DeleteImpl(SequenceNumber seq, uint32_t /*column_family_id*/, const Slice& key,
        const Slice& value, ValueType delete_type,
        const ProtectionInfoKVOS64* kv_prot_info) {
        Status ret_status;
        MemTable* mem = cf_mems_->GetMemTable();
        ret_status =
          mem->Add(seq, delete_type, key, value, kv_prot_info,
            concurrent_memtable_writes_, get_post_process_info(mem),
            hint_per_batch_ ? &GetHintMap()[mem] : nullptr);
        if (UNLIKELY(ret_status.IsTryAgain())) {
          assert(seq_per_batch_);
  ...
  Status WriteBatchInternal::InsertInto(
    ...
    MemTableInserter inserter(
      ...
      concurrent_memtable_writes, nullptr /* prot_info */,
      nullptr /*has_valid_writes*/, seq_per_batch, batch_per_txn);
    std::map<int, LBA> lba_map = write_group.leader->batch->get_LBA_info();
    std::map<int, PPA> ppa_map = write_group.leader->batch->get_PPA_info();
    for (auto w : write_group) {
      if (w->CallbackFailed()) {
        continue;
      }
      ...
      inserter.set_log_number_ref(w->log_ref);
      inserter.set_prot_info(w->batch->prot_info_.get());
      w->status = w->batch->Iterate(&inserter, l2p_map, lba_map, ppa_map);
      if (!w->status.ok()) {
        return w->status;
      }
      assert(!seq_per_batch || w->batch_cnt != 0);
  ...
  Status WriteBatchInternal::Append(WriteBatch* dst, const WriteBatch* src,
    ...
    for (const auto& pair : src->get_segment_data()) {
      int segment_id = pair.first;
      const std::string& data = pair.second;
      dst->Append_segment(segment_id, data);
    }
  ...
  Status WriteBatch::Split_rep_to_map(int min_size, SequenceNumber& s, std::vector<int> wps) {
    Slice input(rep_);
      ...
      case kTypeValue:
        if (GetLengthPrefixedSlice(&input, &key) &&
          ...
          if (value.size() > min_size) {
            uint64_t key_to_num;
            if (config::is_bench) {
              key_to_num = ParseKey(key.ToString(), 0, 16);
            }else{
              key_to_num = std::stoull(key.ToString());
            }
            int segment_id = key_to_segment_id(key_to_num);
            std::string data;
            PutLengthPrefixedSlice(&data, key);
            PutLengthPrefixedSlice(&data, value);
            char* heard = new char[33];
            uint64_t length = data.size();
            EncodeFixed64(heard + 4, length);
            EncodeFixed64(heard + 12, s);
            EncodeFixed32(heard + 20, segment_id);
            EncodeFixed64(heard + 24, wps[segment_id]);
            wps[segment_id]++;
            heard[32] = kTypeValue;
            uint32_t crc_header = crc32c::Value(heard + 12, 21);
            uint32_t crc = crc32c::Extend(crc_header, data.data(), data.size());
            crc = crc32c::Mask(crc);
            EncodeFixed32(heard, crc);
            std::string str;
            str.append(heard, 33);
            str.append(data.data(), data.size());
            int kPageSize = config::page_size;
            size_t pad = kPageSize - (str.size() % kPageSize);
            if (pad != kPageSize) {  
              str.append(pad, '\0');  
            }
            Append_segment(segment_id, str);
            delete[] heard;
          }
          else {
            std::string data;
            PutLengthPrefixedSlice(&data, key);
            PutLengthPrefixedSlice(&data, value);
            char* heard = new char[17];
            uint64_t length = data.size();
            EncodeFixed64(heard, length);
            EncodeFixed64(heard + 8, s);
            heard[16] = kTypeValue;
            std::string str;
            str.append(heard, 17);
            str.append(data.data(), data.size());
            Append_segment(config::max_open_zones, str);
            delete[] heard;
          }
          s++;
        }
