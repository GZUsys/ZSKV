...
namespace ROCKSDB_NAMESPACE {
  ...
  Status BuildTable(
    ...
    InternalStats::CompactionStats* flush_stats, SegmentManager* segment_manager) {
    assert((tboptions.column_family_id ==
      TablePropertiesCollectorFactory::Context::kUnknownColumnFamily) ==
      tboptions.column_family_name.empty());
    ...
    if (iter->Valid() || !range_del_agg->IsEmpty()) {
      ...
      CompactionIterator c_iter(
        ...
        true /* must_count_input_entries */, segment_manager,
      ...
      for (; c_iter.Valid(); c_iter.Next()) {
        ...
        std::string value_buffer(value.data(), value.size());
        ParsedInternalKey ikey = c_iter.ikey();
        key_after_flush_buf.assign(key.data(), key.size());
        Slice key_after_flush;
        Slice value_after_flush;
        if (ikey.type == kTypeDeletion) {
          ...
          is_kv_separated type =
            static_cast<is_kv_separated>(static_cast<uint8_t>(value.data()[1]));
          value_buffer[0] = 0;
          Slice new_value(value_buffer);
          key_after_flush = key_after_flush_buf;
          if (type == not_value_separated) {
            value_after_flush = new_value;
          }
          else if (type == is_value_separated) {
            new_value.remove_prefix(2);
            uint64_t length = 0;
            uint64_t segment_id = 0;
            uint64_t start_page = 0;
            if (!GetVarint64(&new_value, &length))
              return Status::Corruption("bad internal key1");
            ;
            if (!GetVarint64(&new_value, &segment_id))
              return Status::Corruption("bad internal key2");
            ;
            if (!GetVarint64(&new_value, &start_page))
              return Status::Corruption("bad internal key3");
            ;
            std::string value_with_type;
            value_with_type.push_back(0);                     
            value_with_type.push_back(is_value_separated);  
            PutVarint64(&value_with_type, length);
            PutVarint64(&value_with_type, segment_id);
            PutVarint64(&value_with_type, start_page);
            value_after_flush = value_with_type;
          }
          else {
            return Status::Corruption("bad internal key4");
          }
        }
