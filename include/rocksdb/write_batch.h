 ...
 public:
  ...
  class Handler {
   public:
    ...
    virtual Status PutCF(SequenceNumber seq, uint32_t column_family_id, const Slice& key,
      const Slice& value) {
if (column_family_id == 0) {
// Put() historically doesn't return status. We didn't want to be
// backwards incompatible so we didn't change the return status
...
"non-default column family and PutCF not implemented");
    ...
    virtual Status DeleteCF(SequenceNumber seq, uint32_t column_family_id, const Slice& key) {
      if (column_family_id == 0) {
        Delete(key);
        return Status::OK();
      }
  ...
  Status Iterate(Handler* handler,L2PMap& l2p_map,
  ...
  void Append_segment(int id, const std::string& data) {
    segment_data[id].append(data);
  }

  void Append_LBA(int id, const LBA& lba) { LBA_info[id] = lba; }

  void Append_PPA(int id, const PPA& ppa) { PPA_info[id] = ppa; }

  std::map<int, std::string> get_segment_data() const { return segment_data; }

  std::map<int, LBA> get_LBA_info() const { return LBA_info; }

  std::map<int, PPA> get_PPA_info() const { return PPA_info; }

  std::string get_rep() const { return rep_; }

  Status Split_rep_to_map(int min_size, SequenceNumber& s,std::vector<int> wps);
 ...
 private:
  ...
  std::map<int, std::string> segment_data;

  std::map<int, LBA> LBA_info;

  std::map<int, PPA> PPA_info;
